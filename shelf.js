...
function binAt(){return shelf.data?.mode==='simple'?null:selection?shelf.data?.matrix[selection.r]?.[selection.c]:null;}
function bins(){const result=[];shelf.data.matrix.forEach((row,r)=>row.forEach((bin,c)=>{if(bin)result.push({bin,r,c});}));return result;}
function owner(r,c){return bins().find(p=>r>=p.r&&r<p.r+p.bin.h&&c>=p.c&&c<p.c+p.bin.w);}
function fits(bin,r,c,ignore=selection){
  if(![r,c,bin.w,bin.h].every(Number.isInteger)||r<0||c<0||bin.w<1||bin.h<1||r+bin.h>shelf.data.rows||c+bin.w>shelf.data.cols)return false;
  return !bins().some(p=>!(ignore&&p.r===ignore.r&&p.c===ignore.c)&&overlaps({x:c,y:r,w:bin.w,h:bin.h},{x:p.c,y:p.r,w:p.bin.w,h:p.bin.h}));
}
function choose(r,c){selection={r,c};render();}
function renderGrid(){
  grid.replaceChildren();grid.style.setProperty('--cols',shelf.data.cols);grid.style.setProperty('--rows',shelf.data.rows);grid.style.setProperty('--cell',cellSize+'px');
  const occupied=new Set();bins().forEach(({bin,r,c})=>{for(let rr=r;rr<r+bin.h;rr++)for(let cc=c;cc<c+bin.w;cc++)occupied.add(`${rr},${cc}`);});
  for(let r=0;r<shelf.data.rows;r++)for(let c=0;c<shelf.data.cols;c++){
    if(occupied.has(`${r},${c}`))continue;
    const cell=document.createElement('button');cell.className='shelf-cell'+(selection?.r===r&&selection?.c===c?' selected':'');cell.setAttribute('aria-label',`Empty cell, row ${r+1}, column ${c+1}`);cell.style.gridArea=`${r+1} / ${c+1}`;cell.addEventListener('click',()=>choose(r,c));grid.append(cell);
  }
  ...
  bins().forEach(({bin,r,c})=>{
    ...
    gridPosition(el,{x:c+1,y:r+1,w:bin.w,h:bin.h});el.style.backgroundColor=bin.background;el.style.setProperty('--bg',bin.background);el.title=`${bin.name}\n${bin.contents}`;el.setAttribute('aria-label',`${bin.name || 'Unnamed bin'}, row ${r+1}, column ${c+1}`);el.append(labelFor(bin));el.addEventListener('click',()=>choose(r,c));
    el.addEventListener('pointerdown',event=>{
      if(!isAdmin||event.button!==0)return;
      const startX=event.clientX,startY=event.clientY;let target={r,c},moved=false;
      el.setPointerCapture(event.pointerId);
      const move=e=>{target={r:r+Math.round((e.clientY-startY)/(cellSize+1)),c:c+Math.round((e.clientX-startX)/(cellSize+1))};moved=target.r!==r||target.c!==c;if(moved&&fits(bin,target.r,target.c,{r,c})){gridPosition(el,{x:target.c+1,y:target.r+1,w:bin.w,h:bin.h});el.classList.add('dragging');}};
      ...
      const end=()=>{cleanup();selection={r,c};if(moved&&fits(bin,target.r,target.c,{r,c})){checkpoint();shelf.data.matrix[r][c]=null;shelf.data.matrix[target.r][target.c]=bin;selection=target;changed();}else if(moved)message('Bins cannot overlap or extend outside this shelf.',true);render();};
...
$('#addBin').addEventListener('click',()=>{
  ...
  const bin={...defaults(),name:'New bin',contents:'',keywords:'',w:2,h:2};if(!fits(bin,selection.r,selection.c))bin.w=bin.h=1;
  checkpoint();shelf.data.matrix[selection.r][selection.c]=bin;render();changed();
});
fields.forEach(key=>$('#'+key).addEventListener('change',()=>{
  ...
  checkpoint();shelf.data.matrix[selection.r][selection.c]=null;shelf.data.matrix[target.r][target.c]=candidate;selection=target;render();changed();
}));
$('#delete').addEventListener('click',()=>{if(!isAdmin||!binAt())return;checkpoint();shelf.data.matrix[selection.r][selection.c]=null;render();changed();});
...
function validateImport(data){
  ...
  data.matrix.forEach((row,r)=>{
    ...
    row.forEach((bin,c)=>{if(bin===null)return;
      if(typeof bin!=='object'||!['name','contents','keywords'].every(k=>typeof bin[k]==='string')||bin.name.length>500||bin.contents.length>10000||bin.keywords.length>2000||!Number.isInteger(bin.w)||!Number.isInteger(bin.h)||bin.w<1||bin.h<1||r+bin.h>data.rows||c+bin.w>data.cols||!Number.isInteger(bin.fontSize)||bin.fontSize<6||bin.fontSize>96||!/^#[0-9a-f]{6}$/i.test(bin.background)||!/^#[0-9a-f]{6}$/i.test(bin.color)||!['system-ui','Arial','Georgia','monospace'].includes(bin.fontFamily)||typeof bin.bold!=='boolean')throw new Error('Invalid bin data, dimensions, or style.');
      for(let rr=r;rr<r+bin.h;rr++)for(let cc=c;cc<c+bin.w;cc++){const key=`${rr},${cc}`;if(used.has(key))throw new Error('Imported bins overlap.');used.add(key);}
    });
  });
}
...
document.addEventListener('keydown',event=>{
  if(!isAdmin||event.ctrlKey||event.metaKey||['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName))return;
  const bin=binAt(),delta={ArrowLeft:[0,-1],ArrowRight:[0,1],ArrowUp:[-1,0],ArrowDown:[1,0]}[event.key];
  if(!bin||!delta)return;event.preventDefault();const r=selection.r+delta[0],c=selection.c+delta[1];
  if(fits(bin,r,c)){checkpoint();shelf.data.matrix[selection.r][selection.c]=null;shelf.data.matrix[r][c]=bin;selection={r,c};render();changed();}else message('That matrix position is occupied or outside the shelf.',true);
});
...
else {$('#subtitle').textContent=`${shelfId} · Independent shelf matrix`;$('#file').textContent=`data/shelves/${shelfId}.json`;startApp(shelf);api('/api/lab').then(data=>{const item=data.items.find(i=>i.id===shelfId);if(item)$('#subtitle').textContent=`${item.name} · ${shelfId} · ${item.area||'Lab'}`;}).catch(()=>{});}
