...
function closeExplorer(immediate=false){
  ...
  explorerRequest++;clearTimeout(explorerCloseTimer);
...
function inspectSection(section){
  ...
  const contained=lab.data.items.filter(item=>item.kind!=='section'&&item.x>=section.x&&item.y>=section.y&&item.x+item.w<=section.x+section.w&&item.y+item.h<=section.y+section.h);
  host.append(title,infoField('Area',section.area||section.name),infoField('Access',section.restricted?'No access — restricted area':'Lab section'),infoField('In this section',contained.length?contained.map(item=>item.name).join('\n'):'No listed items.'));openExplorer();
}
function inspectItem(item){
  if(isAdmin)return;explorerRequest++;const host=$('#explorerContent');host.replaceChildren();const title=document.createElement('h2');title.textContent=item.name;host.append(title,infoField('Type',item.kind),infoField('Area',item.area||'Lab'),infoField('Location ID',item.id));openExplorer();
}
