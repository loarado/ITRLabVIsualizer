...
const lab={data:null,endpoint:'/api/lab',filename:'lab.json',render,onAuthChanged:refreshShelfIndex};
const plan=$('#plan');
const colors={section:'#e3f1fa',shelf:'#fff4d8',table:'#f0f8fd',cart:'#eff8e9',machine:'#fceeee',wall:'#334155',text:'#ffffff'};
...
function validPosition(item){
  ...
  return !d.items.some(other=>other.id!==item.id&&other.kind!=='section'&&overlaps(item,other));
}
function choose(id){const target=lab.data.items.find(i=>i.id===id);if(!isAdmin&&target?.kind==='shelf'){inspectShelf(target);return;}if(!isAdmin&&target?.kind==='section'){inspectSection(target);return;}if(!isAdmin&&target){inspectItem(target);return;}if(isAdmin&&!canEditItem(target)||target?.kind==='section'&&!canEditMap())return;selectedVertex=null;selectedRoute=null;selectedId=id;render();}
...
function bindDrag(element,item){
  element.addEventListener('pointerdown',event=>{
    ...
    const move=e=>{
      ...
      element.classList.add('dragging');if(validPosition(candidate))gridPosition(element,candidate);
    };
    const end=e=>{
      ...
      else if(moved)message('Position blocked: keep items inside the lab and clear of other equipment.',true);
      selectedRoute=null;selectedId=item.id;render();
    };
...
const fields=['name','area','x','y','w','h','background','color','fontSize','fontFamily','bold','locked','showLabel','restricted'];
...
function renderDirectory(){
  ...
  const items=lab.data.items.filter(i=>i.kind!=='section'&&(!kind||i.kind===kind)&&`${i.id} ${i.name} ${i.area} ${shelfIndex[i.id]?.searchText??''}`.toLowerCase().includes(query));
  $('#directory').replaceChildren();
  const categories={shelf:'Shelves & storage',table:'Tables',cart:'Carts & seating',machine:'Machines & tools',wall:'Walls',text:'Labels',section:'Sections'};
...
function addLabItem(kind){
  ...
  const item={...defaults(),id:`${kind[0].toUpperCase()}-${Array.from(crypto.getRandomValues(new Uint8Array(4)),n=>n.toString(16).padStart(2,'0')).join('')}`,name:kind==='section'?'New section':`New ${kind}`,kind,x:1,y:1,w:kind==='section'?10:4,h:kind==='section'?6:3,background:colors[kind],color:'#233748',area:'',locked:false};
  if(kind==='section'){item.showLabel=true;item.restricted=false;item.fontSize=18;item.x=35;item.y=20;}
  ...
  checkpoint();lab.data.items.push(item);selectedRoute=null;selectedId=item.id;render();changed();
}
...
$('#applyOutline').addEventListener('click',()=>{
  ...
  if(!Number.isInteger(cols)||!Number.isInteger(rows)||cols<10||cols>200||rows<10||rows>200||!validOutline(outline,cols,rows)||lab.data.items.some(i=>i.x+i.w-1>cols||i.y+i.h-1>rows)||lab.data.routes.some(route=>route.some(p=>p[0]>cols||p[1]>rows))){message('Invalid map outline. Use at least 3 x,y points and a grid large enough for all items and paths.',true);return;}
...
function setZoom(value){zoom=Math.min(3,Math.max(1,value));plan.style.width=`${zoom*100}%`;$('#zoomOut').disabled=zoom===1;$('#zoomIn').disabled=zoom===3;requestAnimationFrame(()=>fitLabels(plan));}
...
document.addEventListener('keydown',event=>{
  if(event.defaultPrevented||document.querySelector('dialog[open]')||!isAdmin||event.ctrlKey||event.metaKey||['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName))return;
